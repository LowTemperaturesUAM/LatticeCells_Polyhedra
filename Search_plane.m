function d = Search_plane(crystal, Coefplane, axPoly)
...
%This suppossedly should give you a very good initial value for d for the
%plane, maybe using the CM of the cell is not a bad idea but for now i'm
%using the maximum value that i can finde for the cell in that direction.
allPoints = vertcat(vertcat(R0{1,1},R0{1,2},R0{1,3}));
d = -max(allPoints * n');
...
while abs(Void_total - Cut_total) >= tol
    ...
    for i = 1:length(Spoly)
        ...
        if abs(Spoly(i).UserData.rAtom(1,1)) <= LatR(1,1) && abs(Spoly(i).UserData.rAtom(1,2)) <= LatR(2,2)...
                && Spoly(i).UserData.rAtom(1,1) >= 0 && Spoly(i).UserData.rAtom(1,2) >= 0 && Spoly(i).UserData.rAtom(1,3) >= 0
    ...
    Void_total = sum(voidVols);
    Cut_total = sum(upCutVols);
    d = d-0.005
